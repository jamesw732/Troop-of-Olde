...
The first several subsections here outline a well-defined logical stack that has
arisen as a result of development. This stack is:
\begin{enumerate}
    \item Project Root
    \item Input Handling
    \item Request layer
    ...
    \item Shared API
\end{enumerate}
Each of these layers is explained in detail below. When developing new features, it
is useful to think in terms of this stack - ask yourself, which functionality belongs
to the input handler? The server API? and so on. This stack enforces downward
dependence - input handling relies on the request layer, but the request layer
does not rely on the input handling.

This does not actually represent the current state of the repository, but it is
an active development task to move towards this structure.
...
The role of the input handling layer is to parse player inputs, update the local
game state through the client API, and send networking requests.

This layer is not yet separated out, but it will be encapsulated in a singleton InputHandler class.
Only clients will create one. This will be the sole class responsible for making networking
requests. Furthermore, UI-based inputs will be handled here. This is a tradeoff, because it means
the behavior of the UI is no longer restricted to the UI modules, but it also means the input
handling all happens in one place. So really, the functionality of the UI should remain in the
UI code, it should just be encapsulated as an API for the input handler to see.

There will eventually be a class that handles NPC logic. Since the whole point of this is to
essentially emulate player inputs, this will go in here.
\subsection{Request Layer}
The request layer is responsible for remote procedure calls sent by the client to
the server. Authoritative game state is updated via the server API and shared API, and
then remote states are updated via the response layer.

The interface to the request layer should be public to the client, but the implementation
should not be.
\subsection{Server API}
The server API is responsible for encapsulating server-specific objects and functions.
\subsection{Response Layer}
The response layer is responsible for remote procedure calls sent by the server to
the client. The local game state is updated via the client API.

The interface to the response layer should be public to the server, but the implementation
does not need to be.
\subsection{Client API}
The client API is responsible for encapsulating client-specific objects and functions.
The UI is part of the client API.
...
The shared API contains objects and functions shared between the client and server.
...
The shared API includes a base Character class that both ServerCharacters and
ClientCharacters should inherit.
