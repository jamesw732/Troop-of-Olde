...
In general, I try to avoid ``imaginary'' imports at all costs. An example of this (which is currently
violated) is that \codeword{Item} has a
method \codeword{handle_stats} that takes in a \codeword{char} parameter, which is expected to be a
\codeword{Character} object. This means that \codeword{Item} depends on \codeword{Character}, and thus to honor
Python's downward dependency requirement, \codeword{Character} should not depend on \codeword{Item}.
While Python's dynamic typing allows \codeword{Character}s and \codeword{Item}s to be used as inputs to
each others methods without worry, this violates principles of clear ownership and makes the project layers
very fuzzy. This is the main guiding principle of the code structure.
